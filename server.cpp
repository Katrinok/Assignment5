...
// Command line: ./tsampgroup20 < portnum >
//
...
const int MAX_SERVER_CONNECTIONS = 10; // Max number of server connections
...
// Extract command strips the command from the buffer of the STX and ETX and returns a string
...
// Wraps the command with STX and ETX
...
{
    std::string response = "SERVERS," + fromgroupID + "," + myServer.ip_address + "," + std::to_string(myServer.port) + ";"; // Wraps the response in SERVERS
    for (const auto &pair : connectionsList)
    { // Iterates over the connectionsList
        Connection *connection = pair.second;
        if (connection->isServer && (connection->groupID != "Unknown") && (connection->ip_address != "None"))
        { // If the instance is a server and if the id is not None
            response += connection->groupID + "," + connection->ip_address + "," + std::to_string(connection->port) + ";";
        }
    }
    return response;
}
// Function that sends keepalive messages to the servers that are connected including how many messages are in the store for them
void keepAliveFunction(fd_set *openSockets, int *maxfds)
{
    while (true)
    {
        ...
        { // Iterate over the connectionsList
            Connection *connection = pair.second;
            ...
            { // If the instance is a server and if the id is not None
                ...
                std::string keepaliveMessage = "KEEPALIVE," + std::to_string(messageCount);                        // Add the number of messages to the keepalive message
                keepaliveMessage = wrapWithSTXETX(keepaliveMessage);                                               // Wraps the keepalive message in STX and ETX
                ssize_t bytes_sent = send(connection->sock, keepaliveMessage.c_str(), keepaliveMessage.size(), 0); // Send the keepalive message
                if (bytes_sent < 0)
                {
                    if (errno == EPIPE)
                    {
...
// Returns the connection instance if it is a server and if it is connected
Connection *isConnectedAsServer(const std::string &groupId)
{
    for (const auto &pair : connectionsList)
    {
        Connection *connection = pair.second;
        if ((connection->groupID == groupId) && (connection->isServer) && (connection->port != -1))
        {                      // Bætti við til að flagga ef við höfum ekki port numer
...
void addToQueue(std::vector<std::string> servers, myServer server)
{ // Takes in a vector of servers with comma seperated tokens, group_id,
...
int getRandomServerSocket(const std::map<int, Connection *> &connectionsList)
{
    ...
    {
        if (pair.second->isServer)
            serverSockets.push_back(pair.first);
    }
                   ...
                   std::string buffer, myServer server)
{
    ...
    { // example  connect 130.208.243.61 4000
                   ...
                   std::string buffer, myServer server)
{
    ...
    else if (tokens[0].compare("LISTSERVERS") == 0)
    {
        ...
        for (auto const &pair : connectionsList)
        {
            ...
            { // Make sure to check if the connection is a server
                msg += connection->groupID + "," + connection->ip_address + "," + std::to_string(connection->port) + "\n";
            }
        }
        if (msg.empty())
        { // checks if the msg is empty to print out no servers connected
    ...
    { // Sends message to a server if it connected or stores it
        ...
        if (connection)
        { // if connected or in connectionlist
    ...
    { // client can send STATUSREQ,TO_GROUP and the server gets STATUSRESP for testing
        std::string msg;
        Connection *connection = isConnectedAsServer(tokens[1]); // check if connected
        if (connection)
        { // if connected or in connectionlist
    ...
    else if (tokens[0].compare("SENDALL") == 0 && (tokens.size() >= 2))
    {
        ...
        {
            Connection *connection = pair.second;
            if ((connection->isServer) && (connection->groupID != server.groupID))
...
int main(int argc, char *argv[])
{
    ...
    myServer myServer("130.208.243.61", this_port, ourGroupID); // Create a server object that holds the information about our server
    ...
    int max_buffer = 5000;      // Maximum message buffer
    ...
    listenSock = open_socket(this_port); // Create a socket for listening to clients
    ...
    finished = false;
    std::thread keepAliveThread(keepAliveFunction, &openSockets, &maxfds); // Create a thread for keepalive
    while (!finished)
    {
        // This code block handles the queue that holds the servers that we want to connect to
        if (!serverQueue.empty() && connectionsList.size() < MAX_SERVER_CONNECTIONS)
        {
            ...
            if (upcomingServer.port == -1 || upcomingServer.ip_address.empty() || upcomingServer.port == myServer.port)
            {
                std::cout << "Invalid data for server " << upcomingServer.groupID << ". Removing from queue." << std::endl;
                serverQueue.pop(); // Remove the server from the queue if the data is invalid
                continue;
            }
            std::cout << "Try connecting to server " << upcomingServer.groupID << " from queue." << std::endl;
            // Try to connect to the server and get the socket
            ...
            if (serverSocket > 0)
            {
                // If connection succeeded, add the server to the connectionsList
                createConnection(serverSocket, upcomingServer.groupID, upcomingServer.ip_address, upcomingServer.port, true);
                FD_SET(serverSocket, &openSockets);      // Add the socket to the openSockets
                maxfds = std::max(maxfds, serverSocket); // Update the maximum file descriptor
            }
            else
            {
                // If connection failed increment the counter and push this server to the back of the queue
                upcomingServer.connectionAttempts++;
                if (upcomingServer.connectionAttempts < 2)
                { // If the connection failed once, push it to the back of the queue
        ...
        else
        {
            ...
            if (FD_ISSET(listenSock, &readSockets))
            {
                ...
                {                                                                  // If we've reached the max server limit
                    int randomServerSock = getRandomServerSocket(connectionsList); // returns a random server socket
                    std::cout << "Disconnected from a server " << connectionsList[randomServerSock]->groupID << " to make space for a new one." << std::endl;
                    closeConnection(randomServerSock, &openSockets, &maxfds); // Close the connection
                    connectionsList.erase(randomServerSock);                  // Assume this function removes the server from the connectionsList
                }

                sendQueryservers(clientSock, myServer); // Sending the QUERYSERVERS command to the incomming connection
                char handshakeBuffer[1025];
                memset(handshakeBuffer, 0, sizeof(handshakeBuffer));                                    // Clear the handshake buffer
                ...
                for (auto const &pair : connectionsList)
                {
                    ...
                    if (FD_ISSET(connection->sock, &readSockets))
                    {
                        ...
                        else if (commandBytes > 0)
                        {
                            ...
                            { // Catching string error that makes the server cras
    ...
    keepAliveThread.join(); // Join the keepalive thread
}

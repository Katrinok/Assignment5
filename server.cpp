...
int connectToServer(const std::string& ip_address, int port, std::string groupID)
{
    int serverSock;
...
std::string QueryserversResponse(const std::string& fromgroupID, const std::string& fromhostIP, int fromport, const std::vector<std::string>& otherServers)
{
    std::string response = "SERVERS," + fromgroupID + "," + fromhostIP + "," + std::to_string(fromport);
...
void clientCommand(int clientSocket, fd_set *openSockets, int *maxfds, 
                  char *buffer, std::string groupID) 
{
    ...
    if((tokens[0].compare("CONNECT") == 0) && (tokens.size() == 3)) // example  connect 130.208.243.61 4000 
    {
        ...
        connectToServer(ip_address, port, groupID);
    }
    ...
    { // senda á hvert og eitt socket þetta message
        std::string msg;
               ...
               for(auto const& pair : clients)
               {
                  ...
                  if(FD_ISSET(client->sock, &readSockets))
                  {
                      ...
                      else
                      {
                          ...
                          clientCommand(client->sock, &openSockets, &maxfds, buffer, groupID);
                      }
                  }
               }
